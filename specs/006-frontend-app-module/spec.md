...
6. **Given** a user forgets their password, **When** they click "Forgot Password?" on login page, **Then** they are directed to password reset flow
7. **Given** a user in Profile page, **When** they click "Change Password" and provide current/new passwords, **Then** their password is updated securely
8. **Given** a user receives a reset token link, **When** they click the link and enter new password, **Then** their password is reset and they can login
...
- **FR-006**: Application MUST support password reset via secure token-based flow
- **FR-006a**: Application MUST provide "Forgot Password?" link on login page
- **FR-006b**: Application MUST provide Forgot Password page for email submission
- **FR-006c**: Application MUST provide Reset Password page that accepts token from URL
- **FR-006d**: Application MUST provide Change Password functionality in user profile
- **FR-006e**: Application MUST validate password strength (minimum 8 characters) client-side
...
- **View/Page**: Represents distinct UI screens
  - Pages: Login, Registration, Forgot Password, Reset Password, Dashboard (JD input + pipeline progress), Results (candidate list), Projects (history), Profile (account management with password change)
...
1. **Password Reset**: Token-based password reset with 1-hour expiration (email token flow implemented)
