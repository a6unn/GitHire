...
6. **Given** an authenticated user wants to change password, **When** they provide current password and new password, **Then** the API validates current password and updates to new password
7. **Given** a user forgets their password, **When** they request password reset with email, **Then** the API generates a secure token valid for 1 hour
8. **Given** a user receives a reset token, **When** they submit token with new password within 1 hour, **Then** the API resets password and invalidates the token
...
- **FR-006a**: System MUST allow authenticated users to change their password with current password verification
- **FR-006b**: System MUST support password reset via email token (forgot password flow)
- **FR-006c**: System MUST generate secure, time-limited password reset tokens (1-hour expiration)
- **FR-006d**: System MUST enforce one-time use for password reset tokens
- **FR-006e**: System MUST validate password strength (minimum 8 characters)
...
- **PasswordResetToken**: Manages password reset requests
  - Attributes: token_id, user_id, token, created_at, expires_at (1 hour), used, used_at
  - Relationships: Belongs to one User
  - Validation: One-time use, expires after 1 hour
